...
3. **Optimise the loadout as a whole.** A gear swap that creates resistance slack elsewhere — legs that cover what two ring augments currently cover — frees those slots. Say what to re-slot them with.
   - **Resistance-complete is not done.** Once every reachable resistance is handled, the remaining budget is offense and the second defensive layer: rank the remaining candidates and the held options by damage along the build's top types (§4's totals) and by Offensive Ability, and weigh Defensive Ability, health, armour and sustain as the defence that matters once resistances are capped. **Sustain is a stat with a number:** §3 states the global \`% of Attack Damage converted to Health\` and names every source, and §2 says what it applies to — on a build whose damage is weapon attacks it is the main sustain, so a swap that removes a source is a defensive cost to name and count; on a skill with no \`% Weapon Damage\` it is worth nothing, and a skill's own figure (§4, *this skill only*) is that skill's alone. A plan whose only content is resistance shuffling, on a character whose resistances were already close, has missed the brief — say what the next damage, Offensive Ability, Defensive Ability or health upgrade is, and where it lives behind a level or attribute threshold, commit it through \`hold\` and \`nextLevels\` rather than leaving it unsaid.
...
**Dispositions are exhaustive for the carried bags, and the stash is off the sell list.** Every piece of gear §7 offers from the character's **bags** — the ranked candidates tagged \`[inv]\` and everything in its "Carried but unranked" list — must end in exactly one place: a \`verdicts\` slot, \`hold\`, or \`sell\`. Silence about a carried item is indistinguishable from never having considered it, and it is checked mechanically. Items tagged \`[stash]\` or \`[transfer]\` are the mirror rule: the player stored them **on purpose**, so never put one in \`sell\` — recommend it for wearing or holding when it earns that, and otherwise say nothing about it (also checked). Materials, components, consumables, blueprints and quest items owe no disposition. And \`nextLevels\` and \`hold\` are two views of one decision: every threshold you commit to names the held items it unlocks, and every held item's threshold appears there. An unlock you advise **skipping** belongs in neither — it is disposed of by the rule above (a carried one is \`sell\`; a stored one is left unmentioned), not given a "skip" row of its own.
