...
- **Conversion-aware damage ranking:** the profile lists conversion lines but ranks raw sums (disclosed in the exclusions since 5A.4); folding global and skill-scoped conversion into the ranking would stop a 100%-converted build from misreading. Related: sum the character's actual attack/cast/move speed totals against `GameDb.speedCaps()` so "already capped" is computed rather than left to the advisor.
