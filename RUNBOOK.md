...
- [x] **Stage 5B — Context document builder** — [docs/plans/stage-05-context-builder.md](docs/plans/stage-05-context-builder.md)
  ...
  Gate: eleven sections at **~29.5k tokens**, matrix identical to `aggregates`, **zero** raw `key: value` fallbacks anywhere in the document. §9 forced a real fix — **the save's faction slot order was guessed and wrong**; it is not in the game data, so it was derived from the live saves (fixed factions at 0–7, `factionUser<N>` at slot **N + 6**) and confirmed five ways, including the Kymon's-Chosen / Death's-Vigil contradiction Stage 1 had flagged. Also: schema 9 keeps `racialBonusRace` and `petBonusName`; skill references follow the activator→buff hop that `db.skillName` alone cannot. The full untrimmed document is **~36k tokens** — `--max-tokens`/`--candidates` are knobs, and 30k is the default rather than a limit. See the plan's Outcome.
